...
import { FileDown, FileText, BookOpen, Mic, Loader2 } from 'lucide-react'
...
export function ExportMenu({ projectId, open: _open, onOpenChange: _onOpenChange }: ExportMenuProps) {
  ...
  const handleExport = async (format: Format) => {
    ...
    try {
      ...
      if (format === 'magazine-qa' || format === 'first-person-narrative' || format === 'manuscript-md') {
        ...
        // PDF / DOCX / Audio — binary response
  ...
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent align="end" className="w-72">
        ...
        <DropdownMenuItem onClick={() => handleExport('audio-qa')} disabled={busy !== null}>
          <Mic className="w-4 h-4 mr-2" />
          <div className="flex flex-col">
            <span>Audio Q&amp;A</span>
            <span className="text-xs text-muted-foreground">MP3 — listen to the interview</span>
          </div>
        </DropdownMenuItem>
        <DropdownMenuSeparator />
        <DropdownMenuLabel className="text-xs uppercase tracking-wider text-muted-foreground">
